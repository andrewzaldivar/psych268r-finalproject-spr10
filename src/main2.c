...
void imThePrincess() {
	...
	while (1)
	{
		...
		else if (rescueBotCount >= BOT_COUNT )//|| isSonarSurrounded)
		{
			motor[motorA] = 0;
			motor[motorB] = 0;
			wait1Msec (5100);
			//Once rescued go silent to kick the rescue bots into disperse mode.
			rescueBotCount = 0; //Reset rescue bot count
			moveAwayFromTrap();
			state = GOTOSOUND;
			break;
		}
		//Check SONAR Surround
		if (!isSonarSurrounded)
		{
			ContinueSonarDetection();
		}
		else if (isSonarSurrounded)
		{
			motor[motorA] = 0;
			motor[motorB] = 0;
