...
task main ()
{
  ...
  //state = TEST;
	while(1)
	{
        ...
        if ( USE_SONAR ) {  // SONAR-based in parallel with pulse-based
          InitSonarDetection();
        }
              ...
              // if using sonar, we also continue being princess
              // if not suffiently surrounded by nearby objects.
              && !isSonarSurrounded ) // AND if not surrounded by things detected by sonar
					{

            if ( USE_SONAR ) {  // SONAR-based in parallel with pulse-based
              // Sonar-based detection should only be on while the princess
              // robot is screaming (playing sound).
              ContinueSonarDetection();
            }

						// Play-mode : short pulse for 500 msec
					  nVolume = 4;
						PlayTone(1184, 100);///Async?!  TODO: cosider using while(bSoundActive){ //do nothing } immediately after this?
						wait1Msec (1100);

            if ( USE_SONAR ) {  // SONAR-based in parallel with pulse-based
              // Turn off the sonar-based behavior while the robot is listening
              // for responses from other robots. (i.e. during "Silent-mode", below)
              PauseSonarDetection();

              // Sonar-based detection will continue again when while loop repeats.
            }
					...
					else
					{
						...
						InitSonarDetection(); //Reset sonar-based detection variables.
						state = DISPERSAL;
						break;
					}
				}
